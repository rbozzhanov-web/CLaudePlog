...
/**
 * Long enough that editing several fields of one flight writes the file once, short enough that
...
async function writeNow(): Promise<void> {
  ...
  writing = true;
  try {
    writeBackup(await listEntries());
  } catch {
    // A failed backup must never take the app down or interrupt what the pilot was doing. The
 ...
 *
 * This is what makes "move to another device" work without the pilot remembering to export:
