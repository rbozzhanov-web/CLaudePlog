...
export default function LogbookScreen() {
  ...
  const jumpTarget = useRef<EntryLocation>(undefined);
  ...
  const jumpToDate = (date: string) => {
    ...
    // A year with no flights falls back to the whole logbook so the nearest entry is still found.
      ...
      <View style={styles.headerRow}>
        <Link href="/import/pick" asChild>
          <Pressable style={styles.headerButton}>
            <Text style={styles.headerButtonText}>Import PDF</Text>
          </Pressable>
        </Link>
        <Pressable style={[styles.headerButton, styles.primaryButton]} onPress={() => router.push('/entry/new')}>
          <Text style={styles.primaryButtonText}>+ New flight</Text>
        </Pressable>
      </View>
          ...
          !loading ? (
            <View style={styles.empty}>
              <Text style={styles.emptyText}>
                {year ? `No flights logged in ${year}.` : 'No flights logged yet.'}
              </Text>
              <Text style={styles.emptySubtext}>
                {year ? 'Pick another year, or All.' : 'Add one manually or import a PDF report.'}
              </Text>
            </View>
          ) : null
        }
          ...
          return (
            ...
            >
              ...
              <Text style={styles.monthTotal}>
                {month.flightCount} · {minutesToHHMM(month.totalMinutes)}
              </Text>
            </GlassSurface>
          );
        }}
        renderItem={({ item }) => {
          if (isYearDivider(item)) {
            return <YearDivider yearHeader={item.yearHeader} />;
          }

          // A training session has no flight time; showing it as "ALA → ALA 00:00" would read
          // like a flight that never moved. Lead with the device instead.
          const isSimulator = item.simulatorMinutes > 0 && item.totalTimeMinutes === 0;
                ...
                <Text style={styles.meta}>
                  ...
                  {isSimulator
                    ...
                    : item.aircraftType
                      ? ` · ${item.aircraftType}`
                      : ''}
                  {!isSimulator && item.aircraftRegistration ? ` ${item.aircraftRegistration}` : ''}
                </Text>
              </View>
              <Text style={[styles.time, isSimulator && styles.simTime]}>
                {minutesToHHMM(isSimulator ? item.simulatorMinutes : item.totalTimeMinutes)}
              </Text>
            </Pressable>
          );
        }}
...
function YearDivider({ yearHeader }: { yearHeader: YearHeader }) {
  const { colors } = useAppTheme();
  const styles = useMemo(() => createStyles(colors), [colors]);
  const { year, flightCount, totalMinutes } = yearHeader;

  return (
    <View style={styles.yearHeader}>
      <Text style={styles.yearLabel}>{year}</Text>
      <Text style={styles.yearTotal}>
        {flightCount} {flightCount === 1 ? 'flight' : 'flights'} · {minutesToHHMM(totalMinutes)}
      </Text>
    </View>
  );
}
...
const createStyles = (c: AppColors) =>
  StyleSheet.create({
    ...
    simBadgeText: { fontSize: 10, fontWeight: '700', color: c.textMuted, letterSpacing: 0.5 },
    simTime: { color: c.textMuted },
