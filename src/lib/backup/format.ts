 ...
 * it does not know rather than guessing: a logbook backs a licence, and half-importing one is
...
export type BackupFile = z.infer<typeof backupSchema> & { entries: FlightLogEntry[] };

export function buildBackup(entries: FlightLogEntry[], exportedAt = new Date().toISOString()): BackupFile {
  return {
    app: BACKUP_APP_ID,
...
export function serializeBackup(entries: FlightLogEntry[], exportedAt?: string): string {
  return JSON.stringify(buildBackup(entries, exportedAt), null, 2);
}
 ...
 * screen, and "Not a Pilot Logbook backup" is a more useful thing to show a pilot than a stack.
 */
 ...
 * must not delete those flights. Because ids survive the round trip, restoring the same file
 * twice is a no-op rather than a doubled logbook.
 */
export function mergeBackup(existing: FlightLogEntry[], incoming: FlightLogEntry[]): MergeResult {
